"use client"

import React, { useEffect, useState } from "react"
import { useParams, useRouter } from "next/navigation"
import axios from "axios"
import CustomVideoPlayer from "@/app/components/video/CustomPleer"
import VideoToVideo from "@/app/components/video/VideoToVideo"
import Header from "@/app/components/body/Header"

type Video = {
  id: number
  videoUrl: string
  videoPreview: string
  videoName: string
  videoDescription: string
  ownerUsername: string
  ownerAvatar: string
  created_at: string
}

export default function VideoPage() {
  const params = useParams()
  const videoNameParam = params.videoName
  const [video, setVideo] = useState<Video | null>(null)
  const [loading, setLoading] = useState(true)
  const router = useRouter()
  const API = process.env.NEXT_PUBLIC_API_ON_BACKEND

  useEffect(() => {
    if (!videoNameParam) return

    const videoName = Array.isArray(videoNameParam) ? videoNameParam[0] : videoNameParam
    const decodedName = decodeURIComponent(videoName)

    axios
      .get<Video>(`${API}/video/by-name/${encodeURIComponent(decodedName)}`, { withCredentials: true })
      .then(res => setVideo(res.data))
      .catch(() => setVideo(null))
      .finally(() => setLoading(false))
  }, [videoNameParam])

  function timeAgo(dateString: string) {
    const now = new Date()
    const past = new Date(dateString)
    const diff = Math.floor((now.getTime() - past.getTime()) / 1000)

    if (diff < 60) return `${diff} секунд назад`
    if (diff < 3600) return `${Math.floor(diff / 60)} минут назад`
    if (diff < 86400) return `${Math.floor(diff / 3600)} часов назад`
    if (diff < 604800) return `${Math.floor(diff / 86400)} дней назад`
    if (diff < 2592000) return `${Math.floor(diff / 604800)} недель назад`
    if (diff < 31536000) return `${Math.floor(diff / 2592000)} месяцев назад`
    return `${Math.floor(diff / 31536000)} лет назад`
  }

  if (loading) return <p className="p-4">Загрузка...</p>
  if (!video) return <p className="p-4">Видео не найдено</p>

  return (
    <>
      <div className="h-full flex p-1 bg-black">
        <h1 className="text-blue-400 flex">
          <p className="font-bold">NilTube</p>ㅤ-ㅤ{video.videoName}
        </h1>
      </div>
      <Header />
      <div className="flex">
        <div className="p-4 space-y-4">
          <div>
            <CustomVideoPlayer src={video.videoUrl} />
          </div>

          <div className="ml-5 mt-2">
            <p className="text-white font-bold mb-3">{video.videoName}</p>
            <div className="flex gap-3 cursor-pointer" onClick={() => router.push(`/profile/${video.ownerUsername}`)}>
              <img
                src={video.ownerAvatar || 'https://cdn-icons-png.flaticon.com/512/9187/9187604.png'}
                alt="Avatar"
                className="w-10 h-10 rounded-[50%] object-cover"
              />
              <div>
                <p className="text-white text-[16px] mt-2 mb-5 cursor-pointer" >{video.ownerUsername}</p>
              </div>
            </div>
            <div>
              <p className="text-white text-xs">Вышел: {timeAgo(video.created_at)}</p>
              <p className="text-white mt-3 w-100 ">{video.videoDescription}</p>
            </div>
          </div>
        </div>
        <VideoToVideo />
      </div>
    </>
  )
}
